...
class PConvUNet_light(nn.Module):
    def __init__(self, finetune, in_ch=3):
        super().__init__()

        # 3, 64, 64*5 -> 32, 32, 32
        self.enc_1 = PConvActiv(in_ch, 32, 'down-3')
        # 32, 64, 64*5 -> 128, 32, 32
        self.enc_2 = PConvActiv(32, 64, 'down-3')
        # 64, 16, 16 -> 256, 16, 16
        self.enc_3 = PConvActiv(64, 128, 'down-3')
        self.enc_4 = PConvActiv(128, 128, 'down-3-1')

        self.fuse_0 = _conv_bn_relu(256, 128)
        self.attention_block = build_attention_block()
        self.dec_5 = PConvActiv(128, 128, dec=True, active='leaky')
        self.dec_4 = PConvActiv(128, 64,  dec=False, active='leaky')
        self.dec_3 = PConvActiv(64, 32, dec=True, active='leaky')
        self.dec_2 = PConvActiv(32, 16,  dec=True, active='leaky')
        self.dec_1 = PConvActiv(
            16, 3,   dec=False, bn=False, active='tanh', conv_bias=True)
    ...
    def forward(self, img, mask, fea_m):
        ...
        d3, update_mask3 = self.enc_3(d2, update_mask2)
        d4, update_mask4 = self.enc_4(d3, update_mask3)
        u0 = torch.cat([d4, fea_m], dim=1)
        u0 = self.attention_block(u0)
        u0 = self.fuse_0(u0)
        # print(u0.shape, d9.shape)
        u1, update_masku1 = self.dec_5(
            u0, update_mask4, d3, update_mask3)  # dec=True
        u2, update_masku2 = self.dec_4(u1, update_masku1)
        u3, update_masku3 = self.dec_3(
            u2, update_masku2, d2, update_mask2)  # dec=True
        u4, update_masku4 = self.dec_2(
            u3, update_masku3, img, mask)  # dec=True
        u5, update_masku5 = self.dec_1(u4, update_masku4)

        return u5, update_masku5
