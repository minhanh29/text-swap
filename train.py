...
def main():
    ...
    D3 = Discriminator(in_channels = 10).cuda()
    ...
    D3_solver = torch.optim.Adam(D3.parameters(), lr=cfg.learning_rate, betas = (cfg.beta1, cfg.beta2))
    ...
    try:
        if cfg.ckpt_path is not None:
            ...
            D3.load_state_dict(checkpoint['discriminator3'])
            ...
            D3_solver.load_state_dict(checkpoint['d3_optimizer'])
    ...
    for step in range(cfg.max_iter):
        ...
        i_dt_true = torch.cat((i_t, i_s, t_t, t_sk), dim = 1)
        i_dt_pred = torch.cat((i_t, i_s, o_t, o_sk), dim = 1)
        ...
        o_dt_true = D3(i_dt_true)
        o_dt_pred = D3(i_dt_pred)

        i_vgg = torch.cat((t_f, o_f), dim = 0)

        out_vgg = vgg_features(i_vgg)

        db_real_loss = -torch.mean(torch.log(torch.clamp(o_db_true, cfg.epsilon, 1.0)))
        db_real_loss.backward()

        db_fake_loss = -torch.mean(torch.log(torch.clamp(1.0 - o_db_pred, cfg.epsilon, 1.0)))
        db_fake_loss.backward()
        ...
        if ((step+1) % 1 == 0):
            ...
            out_d = [o_db_pred, o_df_pred, o_dt_pred]
            ...
            l_t_gan, l_t_sk, l_t_l1, l_b_gan, l_b_l1, l_f_gan, l_f_l1, l_f_vgg_per, l_f_vgg_style = detail
            #g_scheduler.step()
        ...
        if ((step+1) % cfg.write_log_interval == 0):
            ...
            print('Iter: {}/{} | Gen: {:.4f} | D_bg: {:.4f} | D_fus: {:.4f} | D_t {:.4f} | TG: {:.4f} | SK: {:.4f} | T: {:.4f} | BG: {:.4f} | B: {:.4f} | FG: {:.4f} | F: {:.4f} | Vgg: {:.4f} | VS: {:.4f}'
                  .format(step+1, cfg.max_iter, g_loss.item(), db_loss.item(), df_loss.item(),
                  dt_loss.item(),
                  l_t_gan.item(),
                 l_t_sk.item(),
                 l_t_l1.item(),
