...
def build_l1_loss_with_mask(x_t, x_o, mask):
    mask_ratio = 1. - mask.view(-1).sum() / float(torch.numel(mask))
...
def build_bg_loss(o_b, o_db_pred, t_b, o_vgg):
    # l_b_gan = build_gan_loss(o_db_pred)
    l_b_gan = torch.mean((1.0-o_db_pred)**2)
    l_b_l1 = build_l1_loss(t_b, o_b)
    l_b_vgg_per, l_b_vgg_style = build_vgg_loss(o_vgg)
    l_b_vgg_per = cfg.lf_theta_2 * l_b_vgg_per
    l_b_vgg_style = cfg.lf_theta_3 * l_b_vgg_style
    l_b = l_b_gan + cfg.lb_beta * l_b_l1

    return l_b, l_b_gan, l_b_l1


# def build_text_conversion_loss(out_g, out_d, labels):
def build_text_conversion_loss(out_g, labels):
    o_sk, o_t, mask_t = out_g
    # o_df_pred, o_dt_pred = out_d
    t_sk, t_t = labels
    ...
    # return 0.1 * l_f_gan + 0.1 * l_t_gan + l_t, l_f_gan, l_t_gan, l_t_l1, l_t_sk
    return l_t, l_t_l1, l_t_sk


def build_generator_loss(out_g, out_d, out_vgg, labels):
    o_sk, o_t, o_b, o_f, mask_t = out_g
    o_db_pred, o_df_pred, o_dt_pred = out_d
    o_vgg = out_vgg
    ...
    l_t_l1 = cfg.lt_alpha * build_l1_loss_with_mask(t_t, o_t, mask_t)
    ...
    return l, [l_t_gan, l_t_sk, l_t_l1, l_b_gan, l_b_l1, l_f_gan, l_f_l1, l_f_vgg_per, l_f_vgg_style]
