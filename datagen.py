...
class datagen_mask(Dataset):
    def __init__(self, cfg, torp = 'train', transforms = None):
        ...
        if(torp == 'train'):
            ...
            self.batch_size = cfg.batch_size
            self.data_shape = cfg.data_shape
            self.name_list = os.listdir(os.path.join(self.data_dir, self.i_s_dir))
...
class datagen_text_conversion(Dataset):
    def __init__(self, cfg, torp = 'train', transforms = None):
    ...
    def __getitem__(self, idx):
        ...
        return [i_t, i_s, mask_t]


class datagen_bg(Dataset):
    def __init__(self, cfg, torp = 'train', transforms = None):
    ...
    def __getitem__(self, idx):
        ...
        i_s = io.imread(os.path.join(cfg.data_dir, cfg.t_f_dir, img_name))
        t_b = io.imread(os.path.join(cfg.data_dir, cfg.t_b_dir, img_name))
        mask_t = io.imread(os.path.join(cfg.data_dir, cfg.mask_t_dir, img_name), as_gray = True)

        return [i_s, t_b, mask_t]
